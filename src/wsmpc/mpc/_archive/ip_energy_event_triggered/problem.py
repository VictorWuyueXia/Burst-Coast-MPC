"""Energy-event triggered split-ratio MPC problem."""
...
from wsmpc.mpc.ip_energy_event_triggered.features import energy_momentum_value_symbolic
...
def solve_candidate(
    ...
    """Solve one energy-event split-ratio candidate."""
...
def build_single_shooting_problem(
    ...
    """Build the event-formulation burst-coast single-shooting NLP."""
...
def _stage_cost(x: Any, u: Any, previous_u: Any, environment: EnvironmentConfig) -> Any:
    """Evaluate the simplified energy, momentum, input, and smoothness cost."""

    torque_limit = environment.pendulum.torque_limit_nm
    normalized_u = u / torque_limit
    normalized_delta_u = (u - previous_u) / torque_limit
    return (
        energy_momentum_value_symbolic(x, environment.pendulum)
        + weights.W_INPUT * normalized_u**2
        + weights.W_DELTA_INPUT * normalized_delta_u**2
    )
